...
export default function NavLinks() {
  return (
    <>
      {/* {links.map((link) => {
        ...
        return (
          <a
            key={link.name}
            href={link.href}
            classNameName="h-[48px]  grow flex-row items-center justify-center gap-2 rounded-md bg-gray-50  p-3 text-sm font-medium hover:bg-sky-100 hover:text-blue-600 md:flex-none md:justify-start md:p-2 md:px-3"
          >
            <LinkIcon classNameName="w-6" />
            <p classNameName="hidden md:block">{link.name}</p>
          </a>
        );
      })} */}
      {/* <div classNameName="flex w-full gap-[5%] bg-slate-400 p-5">
        {links.map((link) => {
          const LinkIcon = link.icon;
          return (
            <a
              key={link.name}
              href={link.href}
              classNameName="h-[48px]  grow flex-row items-center justify-center gap-2 rounded-md  p-3 text-sm font-medium hover:bg-sky-100 hover:text-blue-600 md:flex-none md:justify-start md:p-2 md:px-3"
            >
              <LinkIcon classNameName="w-6" />
              <p classNameName="hidden md:block">{link.name}</p>
            </a>
      ...
      <div>
        <header className="mb-2 w-full bg-white ">
          <div className="relative flex max-w-screen-xl flex-col overflow-hidden px-4 py-4 md:mx-auto md:flex-row md:items-center">
            ...
            >
              ...
              <span className="text-black">Jo's</span>
            </a>
            <input type="checkbox" className="peer hidden" id="navbar-open" />
            <label className="absolute right-7 top-5 cursor-pointer md:hidden">
            ...
            >
              <ul className="flex flex-col items-center space-y-2 md:ml-auto md:flex-row md:space-y-0">
                <li className="text-gray-600 hover:text-blue-600 md:mr-12">
                  <a href="#">Explore</a>
                </li>
                <li className="text-gray-600 hover:text-blue-600 md:mr-12">
                  <a href="#">Explore</a>
